...
def RunForward():
    s = subprocess.Popen(
        str(cmdForward), stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
...
def getAllNumber():
    s = subprocess.Popen(str(cmdConect), stderr=subprocess.PIPE,
...
def main(argv):
    if len(argv) != 2:
        print(
            "you must run in the drozer directory and you must input two arg")
